...
def extract_mixedSequences():
	...
	compare = pickle.load(open("pfam_hypothesis_actual.p", "rb"))
	...
	for rank in experiment.keys():
		...
		pickable[rank] = pfams
	pickle.dump(pickable, open("pfam_hypothesis_mixed.p", "wb"))
def extractSequences():
	...
	for rank in experiment.keys():
		...
		if pfams:
			pickable[rank] = pfams
	pickle.dump(pickable, open("pfam_hypothesis_actual.p", "wb"))
def getSequences():
	sequences = {}
	print ("getting")
	cnt = 0
